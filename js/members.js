let colors = ['piglet', '', 'green', 'rosanna', 'sexy-blue', 'purple-love'];
...
function load(members) {
    let membersHtml = document.getElementById('members');
    console.log('--- HERE');
    let data = [];
    for (let i = 0; i < members.length; i++) {
        let color = colors[i % colors.length];
        data.push(getCard({...members[i], color: color}));
    }
    console.log('--- HERE');
    console.log(data);
    console.log(membersHtml);
    membersHtml.innerHTML = data.join('<br>');
}

fetch('https://script.google.com/macros/s/AKfycbxyn0dMGvOD2mp0u3HYLw5jyBsmdoIqjxWdRTB6bCMFC2icax_E_WlGKp3oIyJKKa12/exec').then((e) => e.json()).then((v) => {
    console.log(v);
    load(v);
});